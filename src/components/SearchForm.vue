...
<template>
  <div class="search-form">
    <div class="search-form__form-area">
      <div class="search-form__items">
        <slot />
      </div>
    </div>
    <div class="search-form__btn-area">
      <button v-if="useReset" type="button" class="btn__primary-line--md icon" @click="emit('clear')">
        <Icon name="reset__line--9e4" width="24" height="24" alt="" area-hidden="true" />
        {{ resetText }}
      </button>
      <button type="button" class="btn__primary--md icon" @click="emit('search')">
